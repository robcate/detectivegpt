...
export async function getConversation(callSid: string) {
  const key = `detectiveConvo:${callSid}`;
  ...
  console.log("Created new detective conversation =>", conversation);
  return conversation;
}

export async function appendUserMessage(callSid: string, userText: string) {
  const key = `detectiveConvo:${callSid}`;
...
export async function appendAssistantMessage(callSid: string, assistantText: string) {
  const key = `detectiveConvo:${callSid}`;
  const convo = await getConversation(callSid);
